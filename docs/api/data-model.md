...
Специальная категория "Без категории" — системная, id известен заранее, назначается транзакциям, для которых не сработало ни правило, ни словарь.
...
| category_id | FK → Category, nullable | null трактуется как "Без категории" (или FK на системную категорию — решим при реализации) |
...
1. category_id nullable + отдельная системная категория "Без категории" — или category_id NOT NULL и всегда указывает на системную категорию по умолчанию? (Второй вариант проще для отчётов — не нужно обрабатывать null отдельно.)
2. Нужны ли пользователю несколько счетов сразу в MVP, или на первом шаге хватит одного счёта по умолчанию (упрощает и API, и экран импорта)?
3. Soft-delete для Transaction/Category (флаг `deleted_at`) или обычное удаление — важно для истории правок при импорте.
